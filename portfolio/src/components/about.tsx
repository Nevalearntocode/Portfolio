...
const AboutMain = ({ onMoreDetails }: { onMoreDetails: () => void }) => (
  <motion.div
    ...
    <Card className="w-full bg-background text-foreground lg:w-3/4 xl:w-1/2">
      ...
      <CardContent className="mt-4 flex flex-col gap-4 rounded-lg shadow-lg">
        ...
        <p className="text-base leading-relaxed">
          ...
          know everything and you will never be ready for everything. There are
          always new problems and you always need to learn new things to solve
          them. I've got the correct mindset now.{" "}
        </p>
...
export default function About() {
  ...
  return (
    <AnimatePresence mode="wait">
      {state === "main" ? (
        <AboutMain key="main" onMoreDetails={() => setState("sub")} />
      ) : (
        <motion.div
          key="sub"
          initial={{ opacity: 0 }}
        ...
        >
          ...
          <div className="grid grid-cols-1 gap-6 p-4 md:h-[calc(100%-4rem)] md:grid-cols-2 md:gap-8 md:p-8">
            <DetailCard
              title="Skills"
              content="Proficient in Django, Next.js, React, and various other web technologies. Experienced in both frontend and backend development."
            />
            <DetailCard
              title="Experience"
              content="Developed multiple full-stack applications, including a real-time chat app, an Airbnb clone, and an e-commerce CMS. Each project has helped me grow as a developer."
            />
            <DetailCard
              title="Education"
              content="Self-taught developer with a passion for continuous learning. Completed numerous online courses and coding bootcamps to stay up-to-date with the latest technologies."
            />
            <DetailCard
              title="Goals"
              content="Seeking opportunities to contribute to innovative projects, collaborate with talented developers, and continue growing my skills in full-stack development."
            />
          </div>
        </motion.div>
      )}
